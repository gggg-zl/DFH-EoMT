...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # Some images (especially in Cityscapes test/val) may have only
        # ignore-in-eval labels, so the target_parser can legitimately
        # return empty masks. In that case, create empty tensors instead
        # of calling torch.stack on an empty list.
        if len(masks) > 0:
