...
class LightningModule(lightning.LightningModule):
    def __init__(
        ...
        if ckpt_path:
            try:
                ...
                # 精确检测检查点版本
                # 新版：包含 network.q.q.weight (DualAttentionQuery)
                # 旧版：只包含 network.q.weight (简单 Embedding)，不包含 network.q.q.weight
                has_new_q = 'network.q.q.weight' in state_dict
                has_old_q = 'network.q.weight' in state_dict and not has_new_q
                
                # 检测当前模型版本
                has_new_model_q = hasattr(network.q, 'q') and hasattr(network.q, 'spatial_q')
                
                if has_old_q and has_new_model_q:
                    # 检查点是旧版，模型是新版 → 替换模型为旧版
                    logging.info("[LightningModule] 检测到旧版检查点，自动将模型切换为旧版架构")
                    embed_dim = network.encoder.backbone.embed_dim
                    num_q = network.num_q
                    network.q = nn.Embedding(num_q, embed_dim)
                    network.use_dual_attention = False
                elif has_new_q and not has_new_model_q:
                    # 检查点是新版，模型是旧版 → 替换模型为新版
                    logging.info("[LightningModule] 检测到新版检查点，自动将模型切换为新版架构 (DualAttentionQuery)")
                    from models.eomt import DualAttentionQuery
                    spatial_weight = getattr(network, 'spatial_weight', 0.1)
                    network.q = DualAttentionQuery(
                        num_q=network.num_q,
                        embed_dim=network.encoder.backbone.embed_dim,
                        spatial_weight=spatial_weight,
                    )
                    network.use_dual_attention = True
                else:
                    logging.info(f"[LightningModule] 检查点与模型架构兼容 (has_new_q={has_new_q}, has_old_q={has_old_q}, has_new_model_q={has_new_model_q})")
            except Exception as e:
                logging.warning(f"[LightningModule] 检查点版本检测失败: {e}")
    ...
    def window_imgs_semantic(self, imgs):
        ...
        for i in range(len(imgs)):
            img = imgs[i]
            print(f"[DEBUG window_imgs_semantic] img.shape: {img.shape}, dtype: {img.dtype}")
            new_h, new_w = self.scale_img_size_semantic(img.shape[-2:])
            print(f"[DEBUG window_imgs_semantic] new_h={new_h}, new_w={new_w}")
