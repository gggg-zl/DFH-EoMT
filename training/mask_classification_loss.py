...
class MaskClassificationLoss(Mask2FormerLoss):
    ...
    def loss_masks(self, masks_queries_logits, mask_labels, indices):
        # Parent's loss_masks is called with num_masks=1, returning loss.sum().
        # We then normalize by the actual number of matched masks (per-batch),
        # with DDP-aware scaling.
        loss_masks = super().loss_masks(masks_queries_logits, mask_labels, indices, 1)

        num_masks = sum(len(tgt) for (_, tgt) in indices)
        num_masks_tensor = torch.as_tensor(
            float(num_masks), dtype=torch.float, device=masks_queries_logits.device
        )

        if dist.is_available() and dist.is_initialized():
            dist.all_reduce(num_masks_tensor, op=dist.ReduceOp.SUM)
            world_size = dist.get_world_size()
            num_masks_tensor = num_masks_tensor / world_size

        # Clamp to avoid division by zero
        num_masks_tensor = torch.clamp(num_masks_tensor, min=1.0)
